...
export class AuthService {
  ...
  public async loginTwitter(callbackUrl = '') {
    const twitterClientId = SettingService.getValueByKey(SETTING_KEYS.TWITTER_LOGIN_CLIENT_ID);
    const twitterLoginClientSecret = SettingService.getValueByKey(SETTING_KEYS.TWITTER_LOGIN_CLIENT_SECRET);
    ...
    return new Promise((resolver, reject) => {
      try {
        consumer.getOAuthAccessToken(
          ...
          oauthTokenSecret,
          oauth_verifier,
          async (error, _oauthAccessToken, _oauthAccessTokenSecret, profile) => {
            if (error) {
            ...
            const newUser = role === 'performer' ? await _this.performerService.socialCreate(data as any)
              : await _this.userService.socialCreate(data as any);
            await _this.createAuthPassword(
              new AuthCreateDto({
                source: role || 'user',
  ...
  public async verifyLoginGoogle(payload: AuthGooglePayload) {
    ...
    const googleLoginClientId = await SettingService.getValueByKey(SETTING_KEYS.GOOGLE_LOGIN_CLIENT_ID);
