...
export class PaymentService {
  constructor(
    ...
    private readonly pay2mService: Pay2mService,
  ...
  private async getCCbillPaymentGatewaySettings() {
    const flexformId = SettingService.getValueByKey(
      SETTING_KEYS.CCBILL_FLEXFORM_ID
    );
    const singleSubAccountNumber = SettingService.getValueByKey(
      SETTING_KEYS.CCBILL_SINGLE_SUB_ACCOUNT_NUMBER
    );
    const recurringSubAccountNumber = SettingService.getValueByKey(
      SETTING_KEYS.CCBILL_RECURRING_SUB_ACCOUNT_NUMBER
    );
      ...
      products: [
        {
          ...
          description: `${subscriptionType} ${
            ...
            subscriptionType === PAYMENT_TYPE.FREE_SUBSCRIPTION
              ? `in ${performer?.durationFreeSubscriptionDays} days`
              : ""
          }`,
  ...
  public async subscribePerformer(
    ...
    //   SettingService.getValueByKey(SETTING_KEYS.PAYMENT_GATEWAY) || "stripe";
    ...
    if (paymentGateway === "ccbill") {
      ...
      const { flexformId, recurringSubAccountNumber, salt } =
  ...
  public async buyTokens(payload: PurchaseTokenPayload, user: UserDto) {
    ...
    const minWalletPrice =
      SettingService.getValueByKey(SETTING_KEYS.MINIMUM_WALLET_PRICE) || 10;
    const maxWalletPrice =
      SettingService.getValueByKey(SETTING_KEYS.MAXIMUM_WALLET_PRICE) || 1000;
    if (totalPrice < minWalletPrice) {
      throw new HttpException(
        `Minimum top up amount is $${minWalletPrice}`,
        422
      );
    }
    if (totalPrice > maxWalletPrice) {
      throw new HttpException(
        `Maximum top up amount is $${maxWalletPrice}`,
        422
      );
    }
    const products = [
    ...
    if (paymentGateway === "ccbill") {
      const { flexformId, singleSubAccountNumber, salt } =
        await this.getCCbillPaymentGatewaySettings();
  ...
  public async ccbillCancelSubscription(id: any, user: UserDto) {
    ...
    if (!subscription.subscriptionId) {
      ...
      await Promise.all([
        this.performerService.updateSubscriptionStat(
    ...
    const [ccbillClientAccNo, ccbillDatalinkUsername, ccbillDatalinkPassword] =
      await Promise.all([
    ...
    const resp = await axios.get(
      `${ccbillCancelUrl}?subscriptionId=${subscriptionId}&username=${ccbillDatalinkUsername}&password=${ccbillDatalinkPassword}&action=cancelSubscription&clientAccnum=${ccbillClientAccNo}`
    );
    ...
    await this.cancelSubscriptionMailer(subscription);
    throw new HttpException(
      "Cancel subscription has been fail, please try again later",
      400
    );
  }
